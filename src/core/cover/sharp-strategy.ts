...
const FALLBACK_GRADIENT = { start: '#e8f3ef', end: '#c9ded6' };
...
function getBuiltinBackgrounds(): string[] {
  const dir = path.resolve(__dirname, '../../../assets/backgrounds');
  try {
...
function estimateFontSize(width: number, lines: string[]): { title: number; author: number; longestChars: number } {
  ...
  return { title, author: Math.max(20, Math.floor(title / 2)), longestChars };
}
...
function buildTextSvg(params: { title: string; author: string; width: number; height: number }): Buffer {
  ...
  const fontFamily = '"Noto Sans CJK SC", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif';
  ...
  const titleText = lines
    .map((line, index) => {
      ...
      return `<text x="50%" y="${y}" text-anchor="middle" font-size="${sizes.title}" font-family="${escapeXml(fontFamily)}" fill="rgba(0,0,0,0.72)">${escapeXml(line)}</text>`;
    })
    .join('');

  const authorText = params.author.trim()
    ? `<text x="${titleRight}" y="${authorY}" text-anchor="end" font-size="${sizes.author}" font-family="${escapeXml(fontFamily)}" fill="rgba(0,0,0,0.72)">${escapeXml(params.author.trim())}</text>`
    : '';
...
export class SharpCoverStrategy implements CoverStrategy {
  name = 'sharp';

  async generate(options: CoverGenerateOptions): Promise<Buffer> {
    const width = options.width;
    const height = options.height;
    const author = options.author?.trim() || 'md2wechat';
    ...
    const compositeInputs: sharp.OverlayOptions[] = [
      ...
      {
        input: buildTextSvg({ title: options.title, author, width, height }),
      },
    ];

    // Add extra overlays if provided
    if (options.overlays) {
...
export function listCoverPresets(): string[] {
  return getBuiltinBackgrounds().map((f) => path.basename(f, path.extname(f)));
}
